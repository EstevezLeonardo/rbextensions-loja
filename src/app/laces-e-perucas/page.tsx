...
interface LacesEPerucasPageProps {
  searchParams: Promise<FiltrosAtuais>;
}

/**
 * Catálogo completo de laces e perucas (api/produtos.php?tipo=laces)
 * com "Critérios de Escolha" (api/filtros.php?tipo=laces, com
 * contagem por opção) — mesmo padrão de /cabelos. Filtros e a busca
 * do header vivem na URL — sem JS de cliente, cada opção é só um link.
 */
export default async function LacesEPerucasPage({ searchParams }: LacesEPerucasPageProps) {
    ...
    <main className="flex flex-1 flex-col">
      <section className="border-b border-zinc-200 bg-zinc-50 px-6 py-10">
        <div className="mx-auto max-w-6xl">
          ...
          <h1 className="mt-1 font-serif text-3xl font-medium text-preto">Laces e Perucas</h1>
          <p className="mt-2 max-w-xl text-sm text-zinc-600">
            Laces e perucas 100% humanas, selecionadas por tom, comprimento e perfil do fio.
          </p>
        </div>
      </section>
      ...
      <section className="mx-auto w-full max-w-6xl px-6 py-10">
        <div className="mb-10">
          <div className="mb-4 flex items-center justify-between">
            ...
            {algumFiltroAtivo && (
              <Link href="/laces-e-perucas" className="text-sm font-semibold text-dourado hover:text-marrom">
                Limpar filtros
              </Link>
            )}
          </div>
...
/**
 * Monta a URL de "/laces-e-perucas" com os filtros atuais + a mudança
 * pedida (chave/valor). Clicar de novo na mesma opção já ativa remove
 * o filtro (alterna); trocar qualquer filtro (menos a própria página)
 * volta pra página 1.
 */
function construirHref(
  ...
  return query ? `/laces-e-perucas?${query}` : "/laces-e-perucas";
}
